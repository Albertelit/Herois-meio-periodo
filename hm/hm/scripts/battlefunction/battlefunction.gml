...
function BattleChangeDefense(_user, _target, _amount, _pmCost, AliveDeadOrEither = 0)
{
    // AliveDeadOrEither: 0 = Vivo  1 = Morto  2 = Qualquer
    var _failed = false;
    ...
    // Texto de mudança de defesa
    instance_create_depth(
        _target.x,
        _target.y,
        _target.depth - 1,
        oBattleFloatingText,
        { font: fnM5x7, col: _col, text: string("defesa reduzida") }
    );

    // Aplica mudança de defesa se não falhou
    if (!_failed)
    {
        // Usando 'defesa' conforme a estrutura da party (sem defesaMax, mínimo 0)
        _target.defesa = max(_target.defesa + _amount, 0);
    }
}